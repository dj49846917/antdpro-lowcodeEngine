...
import { deleteAllStorage, RedirectPlace } from '@/utils/utils';
...
const errorHandler = (error: { request: any, response: any; }) => {
  ...
  if (response && response.status) {
    ...
    if (!request?.options?.url?.inlcudes?.("/oss/file/downloadFile?filePath=materials/assets-private.json")) {
      // history.push('/user/login')
      notification.error({
...
Drequest.interceptors.response.use(async (response) => {
